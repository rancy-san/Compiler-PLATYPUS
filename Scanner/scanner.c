/*
* Purpose: Create a scanner to analyze lexical tokens
	...
	EXAMPLE:
		*/
		...
		if (c == '"') {
			...
			while (1) {
				c = b_getc(sc_buf);
				/* new line, append counter*/
				if (c == '\n')
					line++;
				/* last " found */
				if (c == '"') {
					t.code = STR_T;
					/* from the beginning of the string literal to number of char so far */
					t.attribute.str_offset = b_limit(str_LTBL);
					lexend = b_getcoffset(sc_buf) - 1;

					/* append ... after 17 characters */
					if ((lexend - lexstart) > 20) {
						for (strLen = 0; strLen < 17; strLen++) {
							c = b_getc(sc_buf);
							t.attribute.err_lex[strLen] = c;
						}
						for (strLen = 17; strLen < 20; strLen++) {
							t.attribute.err_lex[strLen] = '.';
						}
						t.attribute.err_lex[strLen] = '\0';
					}
					else {
						for (strLen = 0; strLen < 20; strLen++) {
							c = b_getc(sc_buf);
							t.attribute.err_lex[strLen] = c;
						}
						t.attribute.err_lex[strLen] = '\0';
		...
		*/
		...
		while (lexstart > b_getcoffset(sc_buf)) {
			b_retract(sc_buf);
		}
...
Token aa_func08(char lexeme[]) {
	...
	if ((f > 0 && (f < FLT_MIN || f > FLT_MAX)) || (f < 0 && (f < -FLT_MAX || f > -FLT_MIN))) {
		/*
